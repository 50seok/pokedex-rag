...
> UI 톤 전반 개선: 18개 타입에 커뮤니티 통용 색상 적용(포켓몬/도장 칩 공통), 포켓몬 상세 스탯을 표→막대 그래프(첫 타입 색상)로 변경, 전역 accent를 네이비→레드 계열로 리뉴얼(챗봇 버튼·울음소리 버튼·다음 버튼), 홈에 accent 점+화면풍 결과 패널 추가.
>
> **저작권 판단**(사용자 첨부 실제 도감 기기 사진 참고 요청 관련): 색감·화면 모티프만 오리지널로 반영, 렌즈·키패드 등 특유의 기기 형태는 재현하지 않음 — 기존 IP 정책 유지.
>
> **다음 작업**: PokeAPI 추가 데이터(진화 체인·특성·타입 상성·키/몸무게·서식지) 중 무엇을 추가할지 사용자 확인 대기 — 이 5개는 URL 조합만으론 안 되고 실제 데이터 수집/저장이 필요한 백엔드 작업.

[#28 — feat: 도감 번호 표시 + 상세 페이지 이전/다음 네비게이션 + 울음소리 볼륨 조정](https://github.com/50seok/pokedex-rag/pull/28) (Closes #27) — 2026-09-10

> 로컬 실기동 피드백 3건 반영: 목록/상세에 `#001` 도감 번호 배지, 상세 페이지 ◀이전/다음▶ 네비게이션(id±1, 1/151 경계 자동 숨김), 울음소리 볼륨 1.0→0.5. 순수 템플릿+CSS.
>
> **B안(UI 톤 전반 개선)이 다음 작업으로 대기 중** — 지도/PokeAPI 부가기능(A/C안)과 별개로 시안부터 준비 예정.

[#26 — feat: 포켓몬 상세 페이지에 울음소리 + 움직이는 스프라이트 추가](https://github.com/50seok/pokedex-rag/pull/26) (Closes #25) — 2026-09-10

> 정적 아트워크 옆에 PokeAPI Showdown 애니메이션 gif, "울음소리 듣기" 버튼(cries ogg) 추가. `pokemon.id`로 URL만 템플릿에서 조합 — 백엔드/DB 변경 없음, 로컬 실기동으로 재생·표시 확인 후 머지.
>
> **포트폴리오 완성도 관련 결정**(사용자 확인): 실제 게임풍 일러스트(예: 나무위키의 게임 타일 지도)는 저작권 침해 소지가 커서 배제. 대신 (1) 저비용 PokeAPI 리소스 추가(이 PR), (2) UI 톤 전반 개선(포켓몬 도감다운 색/레이아웃) 예정, (3) 진짜 일러스트가 필요하면 fal.ai 등 이미지 생성 도구 연동 검토 — 사용자가 API 키 준비 중.

[#24 — feat: 마을 목록 페이지에 관동 지도 추가](https://github.com/50seok/pokedex-rag/pull/24) (Closes #23) — 2026-09-09

> `/town` 목록 페이지 상단에 관동 10개 마을 SVG 지도 추가 — 지형 배치형 노드-라인 + 땅/바다 색감, 체육관 유무로 2색 구분, 클릭 시 `/town/{id}` 이동. 좌표는 관동(1세대) 10개 고정 스코프라 DB 컬럼 대신 마크업에 하드코딩(`ponytail:` 주석으로 세대 확장 시 전환 경로 명시). 순수 템플릿+CSS라 컨트롤러/서비스/DB 변경 없음 — code-reviewer 트리거(버그수정/서비스·레포 변경/DB쿼리·마이그레이션) 미해당으로 메인이 직접 자체 검수, 별도 리뷰어 호출 생략. 이 PR에서 pr-gate 트리거가 ~100초 지연되는 현상 관찰(원인 불명, 재시도 없이 자연 복구됨 — 반복되면 재조사 필요).
>
> **스코프 결정**(사용자 확인): 지도 스타일은 원본 게임 이미지 재현이 아니라 다이어그램+색감 강화로 제한(프로젝트 IP 정책과 충돌 피함). 세대 확장은 하지 않고 관동(1세대)만 유지.
